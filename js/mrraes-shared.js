...
const topicLine = safe(S.topic ?? S.topicTitle ?? S.topicName ?? S.subject ?? '');
    ...
    // Extra meta (optioneel, bv. toetsmodus)
    if (Number(S.timeLimitSec) > 0){
      meta.splice(6, 0, ['Tijdslimiet', formatDurationSec(Number(S.timeLimitSec))]);
    }
    if (S.testId) meta.push(['Toets-ID', String(S.testId)]);
    if (S.seed) meta.push(['Seed', String(S.seed)]);
    if (S.hash) meta.push(['Hash', String(S.hash)]);
...
MR_SHARED.askName = function askName(modeOrOpts='taak', maybeOpts){
  ...
  const fixedClass = (opts.fixedClass != null && String(opts.fixedClass).trim()) ? String(opts.fixedClass).trim() : null;
  const hideClass = !!opts.hideClass || !!fixedClass;
...
.mrAskFlag input{width:18px;height:18px}
    ...
    backdrop.innerHTML = `
      <div class="mrAskDlg" role="dialog" aria-modal="true" aria-labelledby="mrAskTitle">
        ...
        <p style="margin:.2rem 0 .8rem; color:#6b7280">Vul je naam in en klik <strong>Start</strong>.</p>
        <div class="mrAskRow" style="grid-template-columns:${hideClass?'1fr':'1fr 1fr'}">
          <input id="mrAskName"  class="mrAskInput" placeholder="Naam" />
          ${hideClass ? '' : '<input id="mrAskClass" class="mrAskInput" placeholder="Klas (bv. 2B)" />'}
        </div>
    ...
    if (inClass) {
      inClass.value = fixedClass || (localStorage.getItem('mr_class') || '').trim();
      if (fixedClass) inClass.disabled = true;
    }
    inName.focus();
    ...
    const ok = () => {
      ...
      const klas = fixedClass || ((inClass && inClass.value) ? String(inClass.value).trim() : '');
      localStorage.setItem('mr_name', nm);
      if (!fixedClass) localStorage.setItem('mr_class', klas);
